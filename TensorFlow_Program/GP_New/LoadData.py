...
def load_data(NNInput):
    ...
    def load_scales(PathToScalingValues):
        ...
        print(('    Loading Scalse from File: ' + PathToScalingValues + '\n'))

        yyData = pandas.read_csv(PathToScalingValues, header=None, skiprows=1)
        yyData = yyData.apply(pandas.to_numeric, errors='coerce')
        #yData  = numpy.transpose(yyData.values)
        yData  = yyData.values
    ...
    xSetTrain = (xData[0:NTrain,:])
    ySetTrain = (yData[0:NTrain,:])

    if (NNInput.RandomizeDataFlg):
        # Shuffle the training set
        order     = numpy.argsort(numpy.random.random(NTrain))
        xSetTrain = xSetTrain[order]
        ySetTrain = ySetTrain[order]


    # if (NNInput.NormalizeDataFlg):
    #     NormalizingMean = xSetTrain.mean(axis=0)
    #     NormalizingSTD  = xSetTrain.std(axis=0)
    #     #PathToScalingValues = NNInput.PathToIniWeightFldr + '/ScalingValues.csv'
    #     #NormalizingMean, NormalizingSTD = load_scales(PathToScalingValues)
    #     xSetTrain = (xSetTrain - NormalizingMean) / NormalizingSTD
    # else:
    #     NormalizingMean = 0.0
    #     NormalizingSTD  = 1.0

    SetTrain = (xSetTrain, ySetTrain)
    ...
    if (NNInput.NormalizeDataFlg):
        if (NNInput.TrainFlg):
            ...
            mean, std = load_scales(PathToScalingValues)
        xSetTrain = (xSetTrain - mean) / std
...
def load_labeled_input_(PathToLabeledInput):
...
def load_print_R(NNInput, Ang):

        PathToTryLabeledInput = NNInput.PathToDataFldr + '/R.csv.' + str(Ang)
        xPrint                = load_labeled_input_(PathToTryLabeledInput)
        return xPrint