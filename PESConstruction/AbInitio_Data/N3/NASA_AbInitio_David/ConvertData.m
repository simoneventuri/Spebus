...
filename = '/Users/sventuri/WORKSPACE/SPES/spes/Data_PES/N3/NASA_AbInitio_David/ACPF_David_NoOutLayer.csv'
delimiter = ',';
startRow = 2;
...
for i=1:size(R1,1)
  ...
  ETriat1(i) = E1(i,1)*27.2113839712790 - VDiatNew;
...
%   if (R1(iPoints,1) >= 10.0) && (R1(iPoints,2) >= 10.0)
%     plot(R1(iPoints,3),E1(iPoints)*27.2113839712790,'ko')
%   elseif (R1(iPoints,1) >= 10.0) && (R1(iPoints,3) >= 10.0)
%     plot(R1(iPoints,2),E1(iPoints)*27.2113839712790,'ro')
%   elseif (R1(iPoints,3) >= 10.0) && (R1(iPoints,2) >= 10.0)
%     plot(R1(iPoints,1),E1(iPoints)*27.2113839712790,'bo')
%   end
%   hold on
 
%   for jPoints=1:size(ANG1,1)
%     if (R1(iPoints,1) == R1(jPoints,1)) && (R1(iPoints,2) == R1(jPoints,2)) && (R1(iPoints,3) == R1(jPoints,3)) && (iPoints ~= jPoints)
%       R1(iPoints,1)
%       R1(iPoints,2)
%       R1(iPoints,3)
%       plot3(R1(iPoints,1),R1(iPoints,2),E1(iPoints)*27.2113839712790,'ko')
%       iEq=iEq+1;
%     end
%   end
%   hold on
...
filename = '/Users/sventuri/WORKSPACE/SPES/spes/Data_PES/N3/NASA_AbInitio_David/MRCI_Q_David.csv';
delimiter = ',';
startRow = 2;
formatSpec = '%s%s%s%s%s%[^\n\r]';
fileID = fopen(filename,'r');
...
for i=1:size(R2,1)
  ...
  ETriat2(i) = E2(i,1)*27.2113839712790 - VDiatNew;
...
%   if (R2(iPoints,1) >= 6.0) && (R2(iPoints,2) >= 6.0)
%     plot(R2(iPoints,3),E2(iPoints)*27.2113839712790,'ko')
%   elseif (R2(iPoints,1) >= 6.0) && (R2(iPoints,3) >= 6.0)
%     plot(R2(iPoints,2),E2(iPoints)*27.2113839712790,'ro')
%   elseif (R2(iPoints,3) >= 6.0) && (R2(iPoints,2) >= 6.0)
%     plot(R2(iPoints,1),E2(iPoints)*27.2113839712790,'bo')
%   end
%   hold on

%   for jPoints=1:size(ANG2,1)
%     if (R2(iPoints,1) == R2(jPoints,1)) && (R2(iPoints,2) == R2(jPoints,2)) && (R2(iPoints,3) == R2(jPoints,3)) && (iPoints ~= jPoints)
%       R2(iPoints,1)
%       R2(iPoints,2)
%       R2(iPoints,3)
%       plot3(R2(iPoints,1),R2(iPoints,2),E2(iPoints)*27.2113839712790,'ko')
%       iEq=iEq+1;
%     end
%   end
%   hold on
...
% R=linspace(1.5,10.0,5000);
% [VOld,dV]=N2_MRCI(R);
% [VNew,dV]=N2_LeRoy(R);
% figure(10)
% plot(R,VOld+9.343749)%+9.805)%9.8992982)
% hold on
% plot(R,VNew+9.8992982)%9.805)%9.8992982)