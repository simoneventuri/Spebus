...
def MorseFunMod(R, Lambda, re):

    p0 = numpy.exp( - Lambda * (R[:,0] - re) )
    p1 = numpy.exp( - Lambda * (R[:,1] - re) )
    p2 = numpy.exp( - Lambda * (R[:,2] - re) )
...
class MorseFun_Layer(lasagne.layers.Layer):

    def __init__(self, incoming, Lambda, re, **kwargs):
        super(MorseFun_Layer, self).__init__(incoming,  **kwargs)
